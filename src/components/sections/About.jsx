        ...
        <div className="grid lg:grid-cols-2 gap-16 lg:gap-24 items-center">
            ...
            <div ref={imgRef} className="absolute inset-[-10%] w-[120%] h-[120%]">
              <div className="w-full h-full panel scanlines p-4 flex flex-col gap-3 relative overflow-hidden">
                ...
                <div className="flex-1 relative overflow-hidden rounded-sm min-h-0">
                  ...
                  />
                  {/* Scan overlay */}
                  <div className="absolute inset-0 pointer-events-none" style={{ background: "linear-gradient(to bottom, transparent 0%, rgba(57,255,20,0.03) 50%, transparent 100%)" }} />
                </div>
          ...
          <div>
            ...
            >
              ONE WATCH.
              <br />
              <span className="text-[var(--green)] text-glow">INFINITE</span>
              <br />
              POWER.
            </motion.h2>

            <motion.p
              className="text-[var(--text-muted)] leading-relaxed mb-5 max-w-lg"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ duration: 0.8, delay: 0.2 }}
            >
              The Omnitrix — the most powerful device in the universe — fell from space and latched onto the wrist of a 10-year-old boy named Ben Tennyson. Built by the genius Galvan scientist Azmuth,
              it contains the DNA of thousands of alien species.
            </motion.p>
            <motion.p
              ...
              {STATS.map(({ label, value }) => (
                <div key={label} className="about-stat opacity-0 panel p-4 group hover:border-[var(--green)]/30 transition-all relative overflow-hidden">
                  <div className="mb-2 w-8 h-8 flex items-center justify-center">
                    {label === "Projects" && (
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#39FF14" strokeWidth="1.5" strokeLinecap="round">
